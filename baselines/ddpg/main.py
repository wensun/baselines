...
import training_LQG as training_LQG
from baselines.ddpg.models import Actor, Critic
from models import Linear_Actor, Quadratic_Critic
...
def run_custom(env_id, seed, noise_type, layer_norm, evaluation, **kwargs):
    # Configure things.
    rank = MPI.COMM_WORLD.Get_rank()
    if rank != 0:
        logger.set_level(logger.DISABLED)
    ...
    # Configure components.
    memory = Memory(limit=int(1e6), action_shape=env.action_space.shape, observation_shape=env.observation_space.shape)
    if env_id.startswith("LQG"): 
        actor = Linear_Actor(nb_actions, layer_norm=layer_norm)
        critic = Quadratic_Critic(layer_norm=layer_norm)
    else:
        actor = Actor(nb_actions, layer_norm=layer_norm)
        critic = Critic(layer_norm=layer_norm)
    ...
    if rank == 0:
        start_time = time.time()
    training_LQG.train(env=env, eval_env=eval_env, param_noise=param_noise,
        action_noise=action_noise, actor=actor, critic=critic, memory=memory, **kwargs)
    env.close()
    if eval_env is not None:
        eval_env.close()
